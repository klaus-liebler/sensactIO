...
BlinkPattern blinkFastRedBlack(CRGB::Red, 300, CRGB::Black, 300); // booting
...
BlinkPattern blinkFastYellowRed(CRGB::Yellow, 300, CRGB::Red, 300); // OTA Process
...
public:
    HAL_SensactOutdoor(otamanager::M *otam) : otam(otam) {}
    ...
    void OutputOneLineStatus()
    {
        ...
        ESP_LOGI(TAG, "Free heap: %d The current date/time in Berlin is %s", esp_get_free_heap_size(), strftime_buf);
    }
    ...
    ErrorCode AfterLoop()
    {
        ...
        if (this->otam->GetState() == otamanager::State::UPDATING)
        {
            strip->AnimatePixel(0, &blinkFastYellowRed);
        }
        else if (newWifiState != lastWifiState)
        {
            switch (newWifiState)
            {
